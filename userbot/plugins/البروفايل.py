...
async def _(event):
    ...
    try:
        ...
        if photo:
            await jmthonevent.edit("الان, يتم التحميل الى تيليكرام ...")
            if photo.endswith((".mp4", ".MP4")):
                # https://t.me/tgbetachat/324694
                size = os.stat(photo).st_size
                if size > 2097152:
                    await jmthonevent.edit("الحجم يجب ان يكون اقل من 2 mb")
                    os.remove(photo)
                    return
                jmthonpic = None
            ...
            try:
                ...
                await edit_or_reply(
                    jmthonevent, "تم بنجاح تغيير صورة حسابي"
                )
    try:
        os.remove(photo)
    except Exception as e:
...
async def count(event):
    ...
    b = 0
    result = ""
    jmthonevent = await edit_or_reply(event, "يتم جمع معلومات حسابك")
...
async def remove_profilepic(delpfp):
    ...
    await edit_delete(
        delpfp, f"تم بنجاح حذف {len(input_photos)} صورة الحساب (s)."
    )


@jmthon.ar_cmd(
    pattern="معرفاتي$",
    command=("معرفاتي", plugin_category),
    info={
