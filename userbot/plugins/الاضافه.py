...
@jmthon.on(admin_cmd(pattern=f"ضيف ?(.*)"))
    ...
    async for user in event.client.iter_participants(JMTHON.full_chat.id):
        try:
            if error.startswith("Too"):
                return (
                    await roz.edit(
                        f"**حـالة الأضـافة انتـهت مـع الأخـطاء**\n- (**ربـما هـنالك ضغـط عـلى الأمࢪ حاول مجددا لاحقـا 🧸**) \n**الـخطأ** : \n`{error}`\n\n• اضافة `{s}` \n• خـطأ بأضافـة `{f}`"
                    ),
                )
            await event.client(
...
@jmthon.ar_cmd(
    pattern="اضافة ([\s\S]*)",
    command=("اضافة", plugin_category),
    info={
        "header": "لاضافة مستخدم في الدردشه التي استخدمت بها الامر.",
        "description": "يتم اضافه فقط عضو واحد وليس جميع الاعضاء",
        "usage": "{tr}اضافة <معرف/ايدي>",
        "examples": "{tr}اضافة @combot @MissRose_bot",
    },
)
async def _(event):
    ...
    await edit_or_reply(event, f"`{to_add_users} تم اضافته بنجاح الى هنا `")
